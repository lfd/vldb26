...
def solve_template(tree_template, card, pred, pred_sel, thresholds_for_join, penalty_value):
    ...
    # Prevent CPs at evaluation_bound
    ...
    current_joins = [0]
    while len(current_joins) > 0:
        current_join_index = current_joins.pop()
        filtered_pred[current_join_index] = []
        current_preds = predecessors[current_join_index]
        for current_pred in current_preds:
            if len(op_list[current_pred]) > 0:
                filtered_pred[current_join_index].append(current_pred)
                current_joins.append(current_pred)
                
    #raw_result = {"costs": np.ceil(costs), "join_operands": op_list, "join_predecessors": predecessors, "leaf_joins": list(leaf_joins.keys()), "opt_time": m.Runtime, "status": m.status, "intermediate_costs": int_costs}
    raw_result = {"costs": np.ceil(costs), "join_operands": op_list, "join_predecessors": filtered_pred, "leaf_joins": list(leaf_joins.keys()), "opt_time": m.Runtime, "status": m.status, "intermediate_costs": int_costs}

    #if not JOUtil.is_valid(op_list, predecessors, 0, num_tables):
    if not JOUtil.is_valid(op_list, filtered_pred, 0, num_tables):
        print(raw_result)
...
def get_threshold_list():
    return [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576, 2097152, 4194304, 8388608]
    

def conduct_experiments_template(benchmarks, max_depth, num_right_leaf_predecessors, problem_path_prefix, result_path_prefix):
    for benchmark in benchmarks:
        queries = os.listdir(path=problem_path_prefix + '/benchmarks/' + benchmark)
        for query in queries:
            query_number = int(query.split('q')[1])
            card, pred, pred_sel = ProblemLoader.get_benchmark_join_ordering_problem(problem_path_prefix + '/benchmarks/' + benchmark + '/' + query, generated_problems=True)
            num_relations = len(card)
            if 0.0 in pred_sel:
                continue
                
            tree_template = generate_template(num_relations, num_right_leaf_predecessors, depth_limit=max_depth)
            tree_template['leaf_joins'] = {}
            ...
            result = solve_template(tree_template, card, pred, pred_sel, thresholds_for_join, 100000)
            
            if result == None:
                print(problem_path_prefix + '/benchmarks/' + benchmark + '/' + query)
            
            result_path = result_path_prefix + '/' + benchmark + '/q' + str(query_number) + '/maxdepth_' + str(max_depth) + '/' + str(num_right_leaf_predecessors) + '_right_pred'
            save_data(result, result_path, 'milp.json')
                
            result_path = result_path_prefix + '/' + benchmark + '/q' + str(query_number)
            save_data(result, result_path, 'milp.json')
...
if __name__ == "__main__":
    ...
    result_path_prefix = 'Experiments/Results/benchmarks'
    
    benchmarks = ['job', 'ldbc', 'tpcds', 'tpch']

    max_depth = 10
    num_right_leaf_predecessors = 0
    conduct_experiments_template(benchmarks, max_depth, num_right_leaf_predecessors, problem_path_prefix, result_path_prefix)
