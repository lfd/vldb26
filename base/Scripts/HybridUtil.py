...
def generate_subproblem(unfinished_operands, graph_type, relations, problem, raw_problem_path):
    problem_string = ''
    if problem < 10:
...
def parse_results_for_subproblems(graph_types, relations, problems, max_depth, num_right_leaf_predecessors, time_limit, raw_problem_path, problem_path_prefix, result_path_prefix, subproblem_path):
    for graph_type in graph_types:
        for i in relations:
            for j in problems:
                ...
                result_path = result_path_prefix + '/' + graph_type + '_graph/' + str(i) + 'relations/' + str(j) + '/maxdepth_' + str(max_depth) + '/' + str(num_right_leaf_predecessors) + '_right_pred/' + str(time_limit) + 's'
                ...
                for leaf_join in raw_result["leaf_joins"]:
                    ...
                    if len(leaf_operands) == 0:
                        continue
                    filtered_raw_problem, contains_cp = generate_subproblem(leaf_operands, graph_type, i, j, raw_problem_path)
                    if contains_cp:
                        cp_occurred = True
                        break
                ...
                for leaf_join in raw_result["leaf_joins"]:
                    ...
                    subproblem_filename = graph_type + '_' + str(i) + '_p' + str(j) + '_md_' + str(max_depth) + '_' + str(num_right_leaf_predecessors) + '_lp_l_' + str(leaf_join) + '_tl_' + str(time_limit)
                    save_data(filtered_raw_problem, subproblem_path, subproblem_filename)
                

         
def derive_total_solutions(graph_types, relations, problems, max_depth, num_right_leaf_predecessors, time_limit, raw_problem_path, problem_path_prefix, raw_result_path_prefix, result_path_prefix, subproblem_solutionpath):
    for graph_type in graph_types:
        for i in relations:
            for j in problems:
                j = int(j)
                ...
                raw_result_path = raw_result_path_prefix + '/' + graph_type + '_graph/' + str(i) + 'relations/' + str(j) + '/maxdepth_' + str(max_depth) + '/' + str(num_right_leaf_predecessors) + '_right_pred/' + str(time_limit) + 's'
                ...
                for leaf_join in raw_result["leaf_joins"]:
                    ...
                    if len(leaf_operands) == 0:
                        continue
                    subproblem_filename = subproblem_solutionpath + '/' + graph_type + '_' + str(i) + '_p' + str(j) + '_md_' + str(max_depth) + '_' + str(num_right_leaf_predecessors) + '_lp_l_' + str(leaf_join) + '_tl_' + str(time_limit) + '-adaptive'
    ...
    for graph_type in graph_types:
        for i in relations:
            for j in problems:
                ...
                best_result = None
                best_config = None
                for (max_depth, num_right_leaf_predecessors) in parameter_configs:
                    for time_interval in time_intervals:
                        result_path = result_path_prefix + '/' + graph_type + '_graph/' + str(i) + 'relations/' + str(j) + '/maxdepth_' + str(max_depth) + '/' + str(num_right_leaf_predecessors) + '_right_pred/' + str(time_interval) + 's'
                        ...
                        if result["costs"] < min_costs:
                            ...
                            best_result = result
                            best_config = (max_depth, num_right_leaf_predecessors)
                ...
                num_right_leaf_predecessors = best_config[1]
